...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "usage: gsc_cli <config.cfg> [--output result.h5]\n";
        return 1;
    }
    ...
    try {
        ...
        const std::string mode_name = "gpu";
        if (output_path.empty()) {
            output_path = gsc::default_result_path(cfg, mode_name);
        }

        // GPU-only mode: CUDA is required
        const auto gpu = gsc::run_case_cuda_u32(cfg);
        if (!gpu.executed) {
            std::cerr << "gpu run failed: " << gpu.message << '\n';
            return 2;
        }
        ...
        // Print detailed timing and memory information
